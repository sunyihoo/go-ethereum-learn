...
func TestBytesConversion1(t *testing.T) {
	bytes := []byte{1, 1, 1, 5}
...
// 该测试用于检查 MixedcaseAddress 对象的自定义 JSON 编组器是否正确调用。
// 在 Go 语言中，结构体指针会继承非指针接收器的方法，但反之则不成立。
// 在 MixedcaseAddress 的情况下，它必须在对象级别（而非指针级别）定义 MarshalJSON 方法，
// 这样这个自定义编组器就可以同时用于 MixedcaseAddress 对象和指针。
