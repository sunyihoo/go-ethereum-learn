...
func TestKeccak256(t *testing.T) {
	msg := []byte("abc")
	exp, _ := hex.DecodeString("4e03657aea45a94fc7d47ba826c8d667c0d1e6e33a64a036ec44f58fa12d6c45")
	checkhash(t, "Sha3-256-array", func(in []byte) []byte { h := Keccak256(msg); return h[:] }, msg, exp)
}

func TestToECDSA(t *testing.T) {
	// Test case 1: Valid 32-byte key
	d := make([]byte, 32)
	d[31] = 1 // Small valid value: 0x01
	priv, err := toECDSA(d, true)
	if err != nil {
		t.Errorf("toECDSA failed for valid key: %v", err)
	}
	if priv.PublicKey.Curve != S256() {
		t.Errorf("Curve mismatch, expected secp256k1")
	}

	// Test case 2: Strict mode, invalid length
	dShort := make([]byte, 31)
	_, err = toECDSA(dShort, true)
	if err == nil {
		t.Errorf("toECDSA should fail for short key in strict mode")
	}

	// Test case 3: Non-strict mode, short key with leading zeros
	_, err = toECDSA(dShort, false)
	if err != nil {
		t.Errorf("toECDSA failed for short key in non-strict mode: %v", err)
	}

	// Test case 4: Key >= N
	dLarge := secp256k1N.Bytes()
	_, err = toECDSA(dLarge, false)
	if err == nil {
		t.Errorf("toECDSA should fail for key >= N")
	}

	// Test case 5: Zero key
	dZero := make([]byte, 32)
	_, err = toECDSA(dZero, true)
	if err == nil {
		t.Errorf("toECDSA should fail for zero key")
	}
}
...
func checkhash(t *testing.T, name string, f func([]byte) []byte, msg, exp []byte) {
	sum := f(msg)
	t.Log("hash:", hex.EncodeToString(sum))
...
func checkAddr(t *testing.T, addr0, addr1 common.Address) {
	t.Log("addr1:", hex.EncodeToString(addr1[:]))
	if addr0 != addr1 {
