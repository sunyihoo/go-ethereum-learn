...
func ReadNodes(it Iterator, n int) []*Node {
	seen := make(map[ID]*Node, n)         // 用于跟踪已见节点，防止重复 // 用于跟踪已见节点，防止重复
	for i := 0; i < n && it.Next(); i++ { // 最多读取 n 个节点 // 最多读取 n 个节点
		// Remove duplicates, keeping the node with higher seq.
		// 移除重复项，保留序列号较高的节点。
		node := it.Node()
		prevNode, ok := seen[node.ID()]
		if ok && prevNode.Seq() > node.Seq() { // 如果已有节点且序列号更高，则跳过 // 如果已有节点且序列号更高，则跳过
			continue
		}
		seen[node.ID()] = node // 更新或添加节点 // 更新或添加节点
	}
	result := make([]*Node, 0, len(seen)) // 创建结果切片 // 创建结果切片
	for _, node := range seen {           // 将所有节点添加到结果中 // 将所有节点添加到结果中
...
func IterNodes(nodes []*Node) Iterator {
	return &sliceIter{nodes: nodes, index: -1} // 返回基于切片的迭代器 // 返回基于切片的迭代器
}
...
func CycleNodes(nodes []*Node) Iterator {
	return &sliceIter{nodes: nodes, index: -1, cycle: true} // 返回循环迭代器 // 返回循环迭代器
}

type sliceIter struct {
	mu    sync.Mutex // 互斥锁，确保线程安全 // 互斥锁，确保线程安全
	nodes []*Node    // 节点列表 // 节点列表
	index int        // 当前索引 // 当前索引
	cycle bool       // 是否循环 // 是否循环
}

func (it *sliceIter) Next() bool {
	it.mu.Lock()         // 加锁 // 加锁
	defer it.mu.Unlock() // 延迟解锁 // 延迟解锁

	if len(it.nodes) == 0 { // 如果节点列表为空，返回 false // 如果节点列表为空，返回 false
		return false
	}
	it.index++                     // 索引递增 // 索引递增
	if it.index == len(it.nodes) { // 如果到达末尾 // 如果到达末尾
		if it.cycle { // 如果是循环模式，重置索引 // 如果是循环模式，重置索引
			it.index = 0
		} else { // 否则清空节点列表并结束 // 否则清空节点列表并结束
			it.nodes = nil
			return false
		}
	}
...
func (it *sliceIter) Node() *Node {
	it.mu.Lock()            // 加锁 // 加锁
	defer it.mu.Unlock()    // 延迟解锁 // 延迟解锁
	if len(it.nodes) == 0 { // 如果节点列表为空，返回 nil // 如果节点列表为空，返回 nil
		return nil
	}
	return it.nodes[it.index] // 返回当前节点 // 返回当前节点
}
...
func (it *sliceIter) Close() {
	...
	it.nodes = nil // 清空节点列表，结束迭代 // 清空节点列表，结束迭代
}
...
func Filter(it Iterator, check func(*Node) bool) Iterator {
	return &filterIter{it, check} // 返回过滤迭代器 // 返回过滤迭代器
}

type filterIter struct {
	Iterator                  // 嵌入基础迭代器 // 嵌入基础迭代器
	check    func(*Node) bool // 过滤条件函数 // 过滤条件函数
}

func (f *filterIter) Next() bool {
	for f.Iterator.Next() { // 遍历基础迭代器 // 遍历基础迭代器
		if f.check(f.Node()) { // 如果节点满足条件，返回 true // 如果节点满足条件，返回 true
			return true
		}
	}
	return false // 没有更多满足条件的节点 // 没有更多满足条件的节点
}
...
type FairMix struct {
	wg      sync.WaitGroup // 等待组，同步源 goroutine // 等待组，同步源 goroutine
	fromAny chan *Node     // 从任意源获取节点的通道 // 从任意源获取节点的通道
	timeout time.Duration  // 超时时间 // 超时时间
	cur     *Node          // 当前节点 // 当前节点

	mu      sync.Mutex    // 互斥锁，保护 sources // 互斥锁，保护 sources
	closed  chan struct{} // 关闭信号通道 // 关闭信号通道
	sources []*mixSource  // 源列表 // 源列表
	last    int           // 上次选择的源索引 // 上次选择的源索引
}

type mixSource struct {
	it      Iterator      // 源迭代器 // 源迭代器
	next    chan *Node    // 源的下一个节点通道 // 源的下一个节点通道
	timeout time.Duration // 源的超时时间 // 源的超时时间
}
...
func NewFairMix(timeout time.Duration) *FairMix {
	m := &FairMix{
		fromAny: make(chan *Node),    // 初始化任意源通道 // 初始化任意源通道
		closed:  make(chan struct{}), // 初始化关闭通道 // 初始化关闭通道
		timeout: timeout,             // 设置超时时间 // 设置超时时间
	}
	return m
}
...
func (m *FairMix) AddSource(it Iterator) {
	m.mu.Lock()         // 加锁 // 加锁
	defer m.mu.Unlock() // 延迟解锁 // 延迟解锁

	if m.closed == nil { // 如果已关闭，返回 // 如果已关闭，返回
		return
	}
	m.wg.Add(1)                                           // 增加等待计数 // 增加等待计数
	source := &mixSource{it, make(chan *Node), m.timeout} // 创建新源 // 创建新源
	m.sources = append(m.sources, source)                 // 添加到源列表 // 添加到源列表
	go m.runSource(m.closed, source)                      // 启动源处理 goroutine // 启动源处理 goroutine
}
...
func (m *FairMix) Close() {
	...
	for _, s := range m.sources { // 关闭所有源迭代器 // 关闭所有源迭代器
		s.it.Close()
	}
	close(m.closed)  // 发送关闭信号 // 发送关闭信号
	m.wg.Wait()      // 等待所有源 goroutine 结束 // 等待所有源 goroutine 结束
	close(m.fromAny) // 关闭任意源通道 // 关闭任意源通道
	m.sources = nil  // 清空源列表 // 清空源列表
	m.closed = nil   // 标记已关闭 // 标记已关闭
}

// Next returns a node from a random source.
// Next 从随机源返回一个节点。
func (m *FairMix) Next() bool {
	m.cur = nil // 重置当前节点 // 重置当前节点

	for { // 无限循环直到成功获取节点或结束 // 无限循环直到成功获取节点或结束
		source := m.pickSource() // 选择下一个源 // 选择下一个源
		if source == nil {       // 如果没有源，从任意通道获取 // 如果没有源，从任意通道获取
			return m.nextFromAny()
		}

		var timeout <-chan time.Time // 超时通道 // 超时通道
		if source.timeout >= 0 {     // 如果超时非负，设置定时器 // 如果超时非负，设置定时器
		...
		select { // 选择操作 // 选择操作
		case n, ok := <-source.next: // 从源获取节点 // 从源获取节点
			if ok {
				...
				m.cur = n // 设置当前节点 // 设置当前节点
				return true
			}
			// This source has ended.
			// 此源已结束。
			m.deleteSource(source) // 删除结束的源 // 删除结束的源
		case <-timeout: // 超时触发 // 超时触发
			...
			source.timeout /= 2
			return m.nextFromAny() // 从任意源获取 // 从任意源获取
		}
	}
}
...
func (m *FairMix) nextFromAny() bool {
	n, ok := <-m.fromAny // 从任意通道获取节点 // 从任意通道获取节点
	if ok {
		m.cur = n // 设置当前节点 // 设置当前节点
	}
	return ok
...
func (m *FairMix) pickSource() *mixSource {
	...
	if len(m.sources) == 0 { // 如果没有源，返回 nil // 如果没有源，返回 nil
		return nil
	}
	m.last = (m.last + 1) % len(m.sources) // 循环选择下一个索引 // 循环选择下一个索引
	return m.sources[m.last]               // 返回选择的源 // 返回选择的源
}
...
func (m *FairMix) deleteSource(s *mixSource) {
	...
	for i := range m.sources { // 查找并删除指定源 // 查找并删除指定源
		if m.sources[i] == s {
...
func (m *FairMix) runSource(closed chan struct{}, s *mixSource) {
	defer m.wg.Done()   // 减少等待计数 // 减少等待计数
	defer close(s.next) // 关闭源通道 // 关闭源通道
	for s.it.Next() {   // 遍历源迭代器 // 遍历源迭代器
		n := s.it.Node() // 获取节点 // 获取节点
		select {         // 选择操作 // 选择操作
		case s.next <- n: // 发送到源通道 // 发送到源通道
		case m.fromAny <- n: // 发送到任意通道 // 发送到任意通道
		case <-closed: // 如果关闭信号触发，退出 // 如果关闭信号触发，退出
			return
		}
	}
}
