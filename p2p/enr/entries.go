...
func (g generic) ENRKey() string { return g.key } // 返回键名

func (g generic) EncodeRLP(w io.Writer) error {
	return rlp.Encode(w, g.value) // 编码值
}

func (g *generic) DecodeRLP(s *rlp.Stream) error {
	return s.Decode(g.value) // 解码值
}
...
func WithEntry(k string, v interface{}) Entry {
	return &generic{key: k, value: v} // 返回通用条目
}
...
func (v ID) ENRKey() string { return "id" } // 返回 "id"
...
func (v IP) ENRKey() string {
	if net.IP(v).To4() == nil {
		return "ip6" // 如果是 IPv6 则返回 "ip6"
	}
	return "ip" // 如果是 IPv4 则返回 "ip"
}
...
func (v IP) EncodeRLP(w io.Writer) error {
	if ip4 := net.IP(v).To4(); ip4 != nil {
		return rlp.Encode(w, ip4) // 编码 IPv4
	}
	if ip6 := net.IP(v).To16(); ip6 != nil {
		return rlp.Encode(w, ip6) // 编码 IPv6
	}
	return fmt.Errorf("invalid IP address: %v", net.IP(v)) // 无效 IP 地址则返回错误
}
...
func (v *IP) DecodeRLP(s *rlp.Stream) error {
	if err := s.Decode((*net.IP)(v)); err != nil {
		return err // 解码失败则返回错误
	}
	if len(*v) != 4 && len(*v) != 16 {
		return fmt.Errorf("invalid IP address, want 4 or 16 bytes: %v", *v) // 长度无效则返回错误
	}
...
func (v IPv4) ENRKey() string { return "ip" } // 返回 "ip"
...
func (v IPv4) EncodeRLP(w io.Writer) error {
	ip4 := net.IP(v).To4()
	if ip4 == nil {
		return fmt.Errorf("invalid IPv4 address: %v", net.IP(v)) // 无效 IPv4 地址则返回错误
	}
	return rlp.Encode(w, ip4) // 编码 IPv4
}
...
func (v *IPv4) DecodeRLP(s *rlp.Stream) error {
	...
	if len(*v) != 4 {
		return fmt.Errorf("invalid IPv4 address, want 4 bytes: %v", *v) // 长度不为 4 则返回错误
	}
	return nil // 成功则返回 nil
}
...
func (v IPv6) ENRKey() string { return "ip6" } // 返回 "ip6"
...
func (v IPv6) EncodeRLP(w io.Writer) error {
	ip6 := net.IP(v).To16()
	if ip6 == nil {
		return fmt.Errorf("invalid IPv6 address: %v", net.IP(v)) // 无效 IPv6 地址则返回错误
	}
	return rlp.Encode(w, ip6) // 编码 IPv6
}
...
func (v *IPv6) DecodeRLP(s *rlp.Stream) error {
	...
	if len(*v) != 16 {
		return fmt.Errorf("invalid IPv6 address, want 16 bytes: %v", *v) // 长度不为 16 则返回错误
	}
	return nil // 成功则返回 nil
}
...
func (v IPv4Addr) ENRKey() string { return "ip" } // 返回 "ip"
...
func (v IPv4Addr) EncodeRLP(w io.Writer) error {
	addr := netip.Addr(v)
	if !addr.Is4() {
		return errors.New("address is not IPv4") // 非 IPv4 地址则返回错误
	}
	enc := rlp.NewEncoderBuffer(w)
	bytes := addr.As4()
	enc.WriteBytes(bytes[:])
	return enc.Flush() // 编码并写入 IPv4
}
...
func (v *IPv4Addr) DecodeRLP(s *rlp.Stream) error {
	...
	if err := s.ReadBytes(bytes[:]); err != nil {
		return err // 读取失败则返回错误
	}
	*v = IPv4Addr(netip.AddrFrom4(bytes)) // 解码为 IPv4 地址
	return nil                            // 成功则返回 nil
}
...
func (v IPv6Addr) ENRKey() string { return "ip6" } // 返回 "ip6"
...
func (v IPv6Addr) EncodeRLP(w io.Writer) error {
	addr := netip.Addr(v)
	if !addr.Is6() {
		return errors.New("address is not IPv6") // 非 IPv6 地址则返回错误
	}
	enc := rlp.NewEncoderBuffer(w)
	bytes := addr.As16()
	enc.WriteBytes(bytes[:])
	return enc.Flush() // 编码并写入 IPv6
}
...
func (v *IPv6Addr) DecodeRLP(s *rlp.Stream) error {
	...
	*v = IPv6Addr(netip.AddrFrom16(bytes)) // 解码为 IPv6 地址
	return nil                             // 成功则返回 nil
}
...
func (err *KeyError) Error() string {
	if err.Err == errNotFound {
		return fmt.Sprintf("missing ENR key %q", err.Key) // 键缺失的错误信息
	}
	return fmt.Sprintf("ENR key %q: %v", err.Key, err.Err) // 其他键相关的错误信息
}

func (err *KeyError) Unwrap() error {
	return err.Err // 返回底层错误
}
...
func IsNotFound(err error) bool {
	var ke *KeyError
	if errors.As(err, &ke) { // 检查是否为 KeyError
		return ke.Err == errNotFound // 如果是未找到错误则返回 true
	}
