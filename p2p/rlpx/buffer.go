...
type readBuffer struct {
	data []byte // 已处理的数据 / Processed data
	end  int    // 未处理数据的结束位置 / End position of unprocessed data
}
...
func (b *readBuffer) reset() {
	...
	// todo 这样做有什么用呢？
	// 拷贝未处理的数据到 data 的前面 / Copy unprocessed data to the front of data
...
type writeBuffer struct {
	data []byte // 待写入的数据 / Data to be written
}
