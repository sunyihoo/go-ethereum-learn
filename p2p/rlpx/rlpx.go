...
type Conn struct {
	dialDest *ecdsa.PublicKey // 拨号目标的公钥
	...
	// 这些是用于 snappy 压缩的缓冲区。
	// 如果它们非 nil，则启用压缩。
	snappyReadBuffer  []byte // 读取缓冲区
	snappyWriteBuffer []byte // 写入缓冲区
}
...
type sessionState struct {
	...
	egressMAC  hashMAC     // 出口 MAC
	ingressMAC hashMAC     // 入口 MAC
	rbuf       readBuffer  // 读取缓冲区
	wbuf       writeBuffer // 写入缓冲区
}

// hashMAC holds the state of the RLPx v4 MAC contraption.
// hashMAC 持有 RLPx v4 MAC 机制的状态。
type hashMAC struct {
	cipher     cipher.Block // 密码块
	hash       hash.Hash    // 哈希
	aesBuffer  [16]byte     // AES 缓冲区
	hashBuffer [32]byte     // 哈希缓冲区
	seedBuffer [32]byte     // 种子缓冲区
}
...
func (c *Conn) SetSnappy(snappy bool) {
	if snappy {
		c.snappyReadBuffer = []byte{}  // 启用压缩时初始化读取缓冲区
		c.snappyWriteBuffer = []byte{} // 启用压缩时初始化写入缓冲区
	} else {
		c.snappyReadBuffer = nil  // 禁用压缩时置空读取缓冲区
		c.snappyWriteBuffer = nil // 禁用压缩时置空写入缓冲区
	}
}
...
func (c *Conn) SetReadDeadline(time time.Time) error {
	return c.conn.SetReadDeadline(time) // 调用底层连接设置读取截止时间
}
...
func (c *Conn) SetDeadline(time time.Time) error {
	return c.conn.SetDeadline(time) // 调用底层连接设置读写截止时间
}
...
func (c *Conn) Read() (code uint64, data []byte, wireSize int, err error) {
	...
	frame, err := c.session.readFrame(c.conn) // 读取帧
	if err != nil {
		return 0, nil, 0, err
	}
	code, data, err = rlp.SplitUint64(frame) // 分割 RLP 编码的帧
	if err != nil {
		// 无效的消息代码
		return 0, nil, 0, fmt.Errorf("invalid message code: %v", err)
	}
	wireSize = len(data) // 记录原始数据大小

	// If snappy is enabled, verify and decompress message.
	// 如果启用了 snappy，验证并解压缩消息。
	if c.snappyReadBuffer != nil {
		var actualSize int
		actualSize, err = snappy.DecodedLen(data) // 获取解压后的大小
		if err != nil {
			return code, nil, 0, err
		}
		if actualSize > maxUint24 {
			return code, nil, 0, errPlainMessageTooLarge // 消息过大
		}
		c.snappyReadBuffer = growslice(c.snappyReadBuffer, actualSize) // 扩展缓冲区
		data, err = snappy.Decode(c.snappyReadBuffer, data)            // 解压缩
	}
	return code, data, wireSize, err
}

func (h *sessionState) readFrame(conn io.Reader) ([]byte, error) {
	h.rbuf.reset() // 重置读取缓冲区

	// Read the frame header.
	// 读取帧头。
	header, err := h.rbuf.read(conn, 32) // 读取 32 字节的帧头
	if err != nil {
		return nil, err
	}
	...
	// Verify header MAC.
	// 验证帧头 MAC。
	wantHeaderMAC := h.ingressMAC.computeHeader(header[:16]) // 计算预期的帧头 MAC
	if !hmac.Equal(wantHeaderMAC, header[16:]) {
		return nil, errors.New("bad header MAC") // MAC 不匹配
	}

	// Decrypt the frame header to get the frame size.
	// 解密帧头以获取帧大小。
	h.dec.XORKeyStream(header[:16], header[:16]) // 解密前 16 字节
	fsize := readUint24(header[:16])             // 读取帧大小
	// Frame size rounded up to 16 byte boundary for padding.
	// 帧大小向上取整到 16 字节边界以便填充。
	rsize := fsize
	if padding := fsize % 16; padding > 0 {
		rsize += 16 - padding // 计算填充后的帧大小
	}

	// Read the frame content.
	// 读取帧内容。
	frame, err := h.rbuf.read(conn, int(rsize)) // 读取帧数据
	if err != nil {
		return nil, err
	}
	...
	// Validate frame MAC.
	// 验证帧 MAC。
	frameMAC, err := h.rbuf.read(conn, 16) // 读取 16 字节的帧 MAC
	if err != nil {
		return nil, err
	}
	wantFrameMAC := h.ingressMAC.computeFrame(frame) // 计算预期的帧 MAC
	if !hmac.Equal(wantFrameMAC, frameMAC) {
		return nil, errors.New("bad frame MAC") // MAC 不匹配
	}

	// Decrypt the frame data.
	// 解密帧数据。
	h.dec.XORKeyStream(frame, frame) // 解密帧
	return frame[:fsize], nil        // 返回解密后的帧数据
}
...
func (c *Conn) Write(code uint64, data []byte) (uint32, error) {
	...
	if len(data) > maxUint24 {
		return 0, errPlainMessageTooLarge // 消息过大
	}
	if c.snappyWriteBuffer != nil {
		...
		c.snappyWriteBuffer = growslice(c.snappyWriteBuffer, snappy.MaxEncodedLen(len(data))) // 扩展缓冲区
		data = snappy.Encode(c.snappyWriteBuffer, data)                                       // 压缩数据
	}

	wireSize := uint32(len(data))                   // 记录原始数据大小
	err := c.session.writeFrame(c.conn, code, data) // 写入帧
	return wireSize, err
}

func (h *sessionState) writeFrame(conn io.Writer, code uint64, data []byte) error {
	h.wbuf.reset() // 重置写入缓冲区

	// Write header.
	// 写入帧头。
	fsize := rlp.IntSize(code) + len(data) // 计算帧大小
	if fsize > maxUint24 {
		return errPlainMessageTooLarge // 帧过大
	}
	header := h.wbuf.appendZero(16)    // 添加 16 字节的帧头
	putUint24(uint32(fsize), header)   // 写入帧大小
	copy(header[3:], zeroHeader)       // 复制零头
	h.enc.XORKeyStream(header, header) // 加密帧头

	// Write header MAC.
	// 写入帧头 MAC。
	h.wbuf.Write(h.egressMAC.computeHeader(header)) // 写入计算的帧头 MAC

	// Encode and encrypt the frame data.
	// 编码并加密帧数据。
	offset := len(h.wbuf.data)
	h.wbuf.data = rlp.AppendUint64(h.wbuf.data, code) // 追加消息代码
	h.wbuf.Write(data)                                // 写入数据
	if padding := fsize % 16; padding > 0 {
		h.wbuf.appendZero(16 - padding) // 添加填充
	}
	framedata := h.wbuf.data[offset:]        // 获取帧数据
	h.enc.XORKeyStream(framedata, framedata) // 加密帧数据

	// Write frame MAC.
	// 写入帧 MAC。
	h.wbuf.Write(h.egressMAC.computeFrame(framedata)) // 写入计算的帧 MAC

	_, err := conn.Write(h.wbuf.data) // 写入所有数据
	return err
}
...
func (m *hashMAC) computeHeader(header []byte) []byte {
	sum1 := m.hash.Sum(m.hashBuffer[:0]) // 获取当前哈希状态
	return m.compute(sum1, header)       // 计算 MAC
}

// computeFrame computes the MAC of framedata.
// computeFrame 计算帧数据的 MAC。
func (m *hashMAC) computeFrame(framedata []byte) []byte {
	m.hash.Write(framedata)              // 更新哈希
	seed := m.hash.Sum(m.seedBuffer[:0]) // 获取哈希值
	return m.compute(seed, seed[:16])    // 计算 MAC
}
...
func (m *hashMAC) compute(sum1, seed []byte) []byte {
	...
	m.cipher.Encrypt(m.aesBuffer[:], sum1) // 加密 sum1
	for i := range m.aesBuffer {
		m.aesBuffer[i] ^= seed[i] // 异或 seed
	}
	m.hash.Write(m.aesBuffer[:])         // 更新哈希
	sum2 := m.hash.Sum(m.hashBuffer[:0]) // 获取哈希值
	return sum2[:16]                     // 返回前 16 字节
}
...
func (c *Conn) Handshake(prv *ecdsa.PrivateKey) (*ecdsa.PublicKey, error) {
	var (
		sec Secrets        // 会话秘密
		err error          // 错误
		h   handshakeState // 握手状态
	)
	if c.dialDest != nil {
		sec, err = h.runInitiator(c.conn, prv, c.dialDest) // 作为发起者执行握手
	} else {
		sec, err = h.runRecipient(c.conn, prv) // 作为接收者执行握手
	}
	if err != nil {
		return nil, err
	}
	c.InitWithSecrets(sec)  // 初始化会话
	c.session.rbuf = h.rbuf // 设置读取缓冲区
	c.session.wbuf = h.wbuf // 设置写入缓冲区
	return sec.remote, err  // 返回远程公钥和错误
}
...
func (c *Conn) InitWithSecrets(sec Secrets) {
	...
	macc, err := aes.NewCipher(sec.MAC) // 创建 MAC 密码
	if err != nil {
		// 无效的 MAC 秘密
		panic("invalid MAC secret: " + err.Error())
	}
	encc, err := aes.NewCipher(sec.AES) // 创建 AES 密码
	if err != nil {
		// 无效的 AES 秘密
	...
	iv := make([]byte, encc.BlockSize()) // 创建全零初始向量
	c.session = &sessionState{
		enc:        cipher.NewCTR(encc, iv),          // 创建 CTR 加密流
		dec:        cipher.NewCTR(encc, iv),          // 创建 CTR 解密流
		egressMAC:  newHashMAC(macc, sec.EgressMAC),  // 创建出口
		ingressMAC: newHashMAC(macc, sec.IngressMAC), // 创建入口
	}
}
...
// 握手常量。
const (
	sskLen = 16                     // ecies.MaxSharedKeyLength(pubKey) / 2    共享密钥长度
	sigLen = crypto.SignatureLength // elliptic S256 / 签名长度
	pubLen = 64                     // 512 bit pubkey in uncompressed representation without format byte / 公钥长度
	shaLen = 32                     // hash length (for nonce etc) / 哈希长度（用于 nonce 等）
...
type Secrets struct {
	AES, MAC              []byte           // AES 和 MAC 密钥
	EgressMAC, IngressMAC hash.Hash        // 出口和入口 MAC 哈希
	remote                *ecdsa.PublicKey // 远程公钥
}

// handshakeState contains the state of the encryption handshake.
// handshakeState 包含加密握手的状态。
type handshakeState struct {
	initiator            bool              // 是否为发起者
	remote               *ecies.PublicKey  // remote-pubk 远程公钥
	initNonce, respNonce []byte            // nonce 发起者和响应者的 nonce
	randomPrivKey        *ecies.PrivateKey // ecdhe-random  随机私钥
...
type authMsgV4 struct {
	...
	InitiatorPubkey [pubLen]byte // 发起者公钥
...
func (h *handshakeState) runRecipient(conn io.ReadWriter, prv *ecdsa.PrivateKey) (s Secrets, err error) {
	...
	authPacket, err := h.readMsg(authMsg, prv, conn) // 读取认证消息
	if err != nil {
		return s, err
	}
	if err := h.handleAuthMsg(authMsg, prv); err != nil { // 处理认证消息
		return s, err
	}

	authRespMsg, err := h.makeAuthResp() // 创建认证响应
	if err != nil {
		return s, err
	}
	authRespPacket, err := h.sealEIP8(authRespMsg) // 密封认证响应
	if err != nil {
		return s, err
	}
	if _, err = conn.Write(authRespPacket); err != nil { // 写入认证响应
		return s, err
	}

	return h.secrets(authPacket, authRespPacket) // 生成秘密
}

func (h *handshakeState) handleAuthMsg(msg *authMsgV4, prv *ecdsa.PrivateKey) error {
	// Import the remote identity.
	// 导入远程身份。
	rpub, err := importPublicKey(msg.InitiatorPubkey[:]) // 导入公钥
	if err != nil {
		return err
	}
	h.initNonce = msg.Nonce[:] // 设置发起者 nonce
	h.remote = rpub            // 设置远程公钥
	...
	if h.randomPrivKey == nil {
		h.randomPrivKey, err = ecies.GenerateKey(rand.Reader, crypto.S256(), nil) // 生成随机私钥
		if err != nil {
			return err
		}
	}
	...
	// Check the signature.
	// 检查签名。
	token, err := h.staticSharedSecret(prv) // 计算静态共享秘密
	if err != nil {
		return err
	}
	signedMsg := xor(token, h.initNonce)                                  // 计算签名消息
	remoteRandomPub, err := crypto.Ecrecover(signedMsg, msg.Signature[:]) // 恢复公钥
	if err != nil {
		return err
	}
	h.remoteRandomPub, _ = importPublicKey(remoteRandomPub) // 导入远程随机公钥
	return nil
}
...
func (h *handshakeState) secrets(auth, authResp []byte) (Secrets, error) {
	ecdheSecret, err := h.randomPrivKey.GenerateShared(h.remoteRandomPub, sskLen, sskLen) // 生成 ECDHE 秘密
	if err != nil {
		return Secrets{}, err
	}
	...
	// 从临时密钥协商中派生基本秘密
	sharedSecret := crypto.Keccak256(ecdheSecret, crypto.Keccak256(h.respNonce, h.initNonce)) // 计算共享秘密
	aesSecret := crypto.Keccak256(ecdheSecret, sharedSecret)                                  // 计算 AES 秘密
	s := Secrets{
		remote: h.remote.ExportECDSA(),                   // 导出远程公钥
		AES:    aesSecret,                                // 设置 AES 密钥
		MAC:    crypto.Keccak256(ecdheSecret, aesSecret), // 设置 MAC 密钥
	}

	// setup sha3 instances for the MACs
	// 为 MAC 设置 sha3 实例
	mac1 := sha3.NewLegacyKeccak256()
	mac1.Write(xor(s.MAC, h.respNonce)) // 写入 MAC 密钥和响应者 nonce 的异或
	mac1.Write(auth)                    // 写入认证消息
	mac2 := sha3.NewLegacyKeccak256()
	mac2.Write(xor(s.MAC, h.initNonce)) // 写入 MAC 密钥和发起者 nonce 的异或
	mac2.Write(authResp)                // 写入认证响应
	if h.initiator {
		s.EgressMAC, s.IngressMAC = mac1, mac2 // 如果是发起者，设置出口和入口 MAC
	} else {
		s.EgressMAC, s.IngressMAC = mac2, mac1 // 如果是接收者，设置出口和入口 MAC
	}
...
func (h *handshakeState) staticSharedSecret(prv *ecdsa.PrivateKey) ([]byte, error) {
	return ecies.ImportECDSA(prv).GenerateShared(h.remote, sskLen, sskLen) // 生成共享秘密
}
...
func (h *handshakeState) runInitiator(conn io.ReadWriter, prv *ecdsa.PrivateKey, remote *ecdsa.PublicKey) (s Secrets, err error) {
	h.initiator = true                         // 设置为发起者
	h.remote = ecies.ImportECDSAPublic(remote) // 导入远程公钥

	authMsg, err := h.makeAuthMsg(prv) // 创建认证消息
	if err != nil {
		return s, err
	}
	authPacket, err := h.sealEIP8(authMsg) // 密封认证消息
	if err != nil {
		return s, err
	}

	if _, err = conn.Write(authPacket); err != nil { // 写入认证消息
		return s, err
	}

	authRespMsg := new(authRespV4)
	authRespPacket, err := h.readMsg(authRespMsg, prv, conn) // 读取认证响应
	if err != nil {
		return s, err
	}
	if err := h.handleAuthResp(authRespMsg); err != nil { // 处理认证响应
		return s, err
	}
...
func (h *handshakeState) makeAuthMsg(prv *ecdsa.PrivateKey) (*authMsgV4, error) {
	...
	signed := xor(token, h.initNonce)                                    // 计算签名消息
	signature, err := crypto.Sign(signed, h.randomPrivKey.ExportECDSA()) // 签名
	if err != nil {
		return nil, err
	}

	msg := new(authMsgV4)
	copy(msg.Signature[:], signature)                                     // 设置签名
	copy(msg.InitiatorPubkey[:], crypto.FromECDSAPub(&prv.PublicKey)[1:]) // 设置发起者公钥
	copy(msg.Nonce[:], h.initNonce)                                       // 设置 nonce
	msg.Version = 4                                                       // 设置版本
	return msg, nil
}

func (h *handshakeState) handleAuthResp(msg *authRespV4) (err error) {
	h.respNonce = msg.Nonce[:]                                    // 设置响应者 nonce
	h.remoteRandomPub, err = importPublicKey(msg.RandomPubkey[:]) // 导入远程随机公钥
	return err
}
...
func (h *handshakeState) makeAuthResp() (msg *authRespV4, err error) {
	...
	msg = new(authRespV4)
	copy(msg.Nonce[:], h.respNonce)                                     // 设置 nonce
	copy(msg.RandomPubkey[:], exportPubkey(&h.randomPrivKey.PublicKey)) // 设置随机公钥
	msg.Version = 4                                                     // 设置版本
	return msg, nil
}
...
func (h *handshakeState) readMsg(msg interface{}, prv *ecdsa.PrivateKey, r io.Reader) ([]byte, error) {
	h.rbuf.reset()   // 重置读取缓冲区
	h.rbuf.grow(512) // 扩展缓冲区

	// Read the size prefix.
	// 读取大小前缀。
	prefix, err := h.rbuf.read(r, 2) // 读取 2 字节前缀
	if err != nil {
		return nil, err
	}
	size := binary.BigEndian.Uint16(prefix) // 解析大小
	...
	// 读取握手包。
	packet, err := h.rbuf.read(r, int(size)) // 读取包
	if err != nil {
		return nil, err
	}
	dec, err := ecies.ImportECDSA(prv).Decrypt(packet, nil, prefix) // 解密包
	if err != nil {
		return nil, err
	}
	...
	err = s.Decode(msg)                               // 解码消息
	return h.rbuf.data[:len(prefix)+len(packet)], err // 返回读取的数据
}
...
func (h *handshakeState) sealEIP8(msg interface{}) ([]byte, error) {
	...
	h.wbuf.appendZero(mrand.Intn(100) + 100) // 添加随机填充

	prefix := make([]byte, 2)
	binary.BigEndian.PutUint16(prefix, uint16(len(h.wbuf.data)+eciesOverhead)) // 设置前缀

	enc, err := ecies.Encrypt(rand.Reader, h.remote, h.wbuf.data, nil, prefix) // 加密
	return append(prefix, enc...), err                                         // 返回前缀和加密数据
}
...
func importPublicKey(pubKey []byte) (*ecies.PublicKey, error) {
	...
	// TODO: 减少无意义的转换
	pub, err := crypto.UnmarshalPubkey(pubKey65) // 反序列化公钥
	if err != nil {
		return nil, err
	}
	return ecies.ImportECDSAPublic(pub), nil // 导入 ECIES 公钥
}
...
func exportPubkey(pub *ecies.PublicKey) []byte {
	...
	if curve, ok := pub.Curve.(crypto.EllipticCurve); ok {
		return curve.Marshal(pub.X, pub.Y)[1:] // 导出公钥
	}
	return []byte{}
}
