...
func TestTypeMethods(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Simple type",
			typ:       Type{Name: "id", Type: "uint256"},
			wantArray: false,
			wantName:  "uint256",
		},
		{
			name:      "Variable array",
			typ:       Type{Name: "list", Type: "address[]"},
			wantArray: true,
			wantName:  "address",
		},
		{
			name:      "Fixed array",
			typ:       Type{Name: "data", Type: "bytes32[4]"},
			wantArray: true,
			wantName:  "bytes32",
		},
		{
			name:      "Invalid array",
			typ:       Type{Name: "bad", Type: "foo[[[["},
			wantArray: true, // 误报
			wantName:  "foo",
		},
		{
			name:      "Leading bracket",
			typ:       Type{Name: "invalid", Type: "[abc"},
			wantArray: false,
			wantName:  "",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.typ.isArray(); got != tt.wantArray {
				t.Errorf("isArray() = %v, want %v for type %q", got, tt.wantArray, tt.typ.Type)
			}
			if got := tt.typ.typeName(); got != tt.wantName {
				t.Errorf("typeName() = %q, want %q for type %q", got, tt.wantName, tt.typ.Type)
			}
		})
	}
}
